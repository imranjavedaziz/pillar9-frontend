...
import { ADMIN_PRACT_NFT_LIST, BASE_URL } from "@/utilities/endpoints";
...
const Page = () => {
  ...
  async function fetchPractitionerData() {
    try {
      ...
      const response = await axios.get(BASE_URL + ADMIN_PRACT_NFT_LIST, {
        headers,
      });
        ...
        <div className="w-full pb-10 pt-5 px-5  rounded-[24px] bg-white">
          {
            practitionerData?.length == 0 ? (
              <div className=" flex justify-center py-10">
                <p className="text-black text-lg max-w-3xl text-center">Oops, you have no NFTs to gift. Don’t worry, you can easily mint one and come back to this page. Just <Link className="font-semibold text-yellow-600" href={"mint-practitioner-nft"}>tap here</Link> to begin the minting process.</p>
              </div>
            ) : (
